...
listenForActionButtons = (controller)->
  ...
  $actionInputCopy = $('#copy-form input')
    ...
    $.ajax
      ...
      success: (data)->
        ...
        $('.footer-inner').animate {left: '-200%'}, ->
          ...
          # Update share link
          $actionCopy.attr('src', URI_ROOT + 'mesaj/' + MESSAGE_ID)
          $actionInputCopy.val(URI_ROOT + 'mesaj/' + MESSAGE_ID)
  ...
  $actionCopy.on 'click', ->
    $(this).toggleClass("active").siblings('.action-box').toggleClass('active')
    $actionInputCopy.focus().select()
  $actionCopy.on 'mouseover', ->
    $actionInputCopy.focus().select()
